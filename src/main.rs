...
async fn main() {
    ...
    match matches.subcommand() {
        Some(("clear-cache", cache_matches)) => {
            ...
            let verbosity = cache_matches.value_of("verbose").unwrap();
        ...
        Some(("search", search_matches)) => {
            // Search on CLI
            let verbosity = search_matches.value_of("verbose").unwrap();
            utils::configure_log(verbosity);
            let engine = search_matches.value_of("engine").unwrap();
        ...
        Some(("api", api_matches)) => {
            // Start API server on port
            let verbosity = api_matches.value_of("verbose").unwrap();
            utils::configure_log(verbosity);
